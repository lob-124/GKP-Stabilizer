...
if __name__ == "__main__":
	...
	for i in well_indices:
		# d = H_LCs[i].shape[0]
		# print("---- WELL {} ----".format(well_nums[i]))
		# # for j in range(d-1):
		# 	print("Matrix element coupling {} and {}: {}".format(j,j+1,abs(H_LCs[i][j][j+1])))
		# 	print("\tCorresponding time scale: {} ps".format((hbar/abs(H_LCs[i][j][j+1]))/picosecond))
		H_LC_window2[well_nums[i]] = H_LCs[i]
		#print("Largest matrix element for well {}: {}".format(well_nums[i],abs(H_LCs[i]-diag(H_LCs[i])).max()))
		

		#argmax = abs(H_LCs[i]-diag(H_LCs[i])).argmax()
		d = H_LCs[i].shape[0]
		max_inds = (0,0)
		_max = 0.0
		for k in range(d):
			for l in range(k+1,d):
				if abs(H_LCs[i][k][l]) > _max:
					max_inds = (k,l)
					_max = abs(H_LCs[i][k][k])
		# print("Largest matrix element: {}".format(_max))	
		# print("\tCoupling well states {} and {}".format(*max_inds))
		# print("\tCorresponding time scale: {} ps".format((hbar/abs(H_LCs[i][max_inds[0]][max_inds[1]]))/picosecond))
		#print("\n\n",end="")
		if i < num_wells-2:
		...
		L1 = 2*pi*sqrt(gamma)*g_mat*X1s[i]
		L2 = 2*pi*sqrt(gamma)*g_mat*X2s[i]

		# print("Jump operator matrix elements: {} and {}".format(abs(L1[max_inds[0],max_inds[1]]),abs(L2[max_inds[0],max_inds[1]])))
		# print("\tCorresponding time scales: {} ps and {} ps".format((hbar/abs(L1[max_inds[0],max_inds[1]]))/picosecond,(hbar/abs(L2[max_inds[0],max_inds[1]]))/picosecond))
	...
	if resistor_window_2:
		SSE_obj = SSE2(E_J,num_periods,max_wells,Us_window1,dUs_window1,L1s_window1,L2s_window1,L1s_window2,L2s_window2,H_LC_window2,overlaps_window2,dt_1,num_tpoints1,dt_2,dt_window2,T_LC,bump_function)
	else:
		SSE_obj = SSE(E_J,num_periods,max_wells,Us_window1,dUs_window1,L1s_window1,L2s_window1,H_LC_window2,overlaps_window2,num_tpoints1,dt_2)
	psi = {}
	d_zero = len(well_energies[zero_well_ind])
	psi[0] = zeros(d_zero,dtype=complex64)
	psi[1] = zeros(d_zero,dtype=complex64)
	psi[-1] = zeros(d_zero,dtype=complex64)

	for i in range(d_zero-5,d_zero):
		psi[-1][i] = 1.0/sqrt(15)
		psi[0][i] = 1.0/sqrt(15)
		psi[1][i] = 1.0/sqrt(15)

	# _n = 2*(max_wells//2)+1
	# for i in range(-max_wells//2,max_wells//2):
	# 	psi[i] = zeros(len(well_energies[zero_well_ind]),dtype=complex64)
	# 	psi[i][5] = 1/sqrt(_n)
	...
	with Pool(processes=num_threads) as p2:
		for psi_mid, psi_end,jump_times in p2.starmap(SSE_obj.SSE_evolution,SSE_params,chunksize=int(ceil(num_samples/num_threads))):
			psi_mids.append(psi_mid)
			psi_ends.append(psi_end)
			jumps.append(jump_times)


	out_arr = array([parameters,psi_mids,psi_ends,jumps],dtype=object)
